...
OTEL_AGENT_SO_PATH=${PWD}/../../_build/${BUILD_ARCHITECTURE}-release/phpbridge_extension/code/phpbridge_${PHP_VERSION}.so
if [ ! -f ${OTEL_AGENT_SO_PATH} ]; then
    echo "Native build not found: '${OTEL_AGENT_SO_PATH}'"
    exit 1
fi
...
docker run --rm \
    ...
    -v ${OTEL_AGENT_SO_PATH}:/otel/phpbridge.so \
    -w /phpt-tests \
