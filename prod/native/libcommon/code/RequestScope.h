...
public:
    ...
    void onRequestInit() {
            ...
            if (!diagnosticFile.empty()) {
                if (sharedMemory_->shouldExecuteOneTimeTaskAmongWorkers()) {
                    try {
                        ...
                        opentelemetry::utils::storeDiagnosticInformation(opentelemetry::utils::getParameterizedString(diagnosticFile), *(bridge_));
        ...
        if (bootstrapSuccessfull_ && (*config_)->inferred_spans_enabled) {
            ...
            if (interval.count() == 0) {
                interval = std::chrono::milliseconds{ConfigurationSnapshot().OTEL_PHP_INFERRED_SPANS_SAMPLING_INTERVAL};
