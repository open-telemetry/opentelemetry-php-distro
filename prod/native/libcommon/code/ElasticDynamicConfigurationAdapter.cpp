...
ElasticDynamicConfigurationAdapter::optionsMap_t ElasticDynamicConfigurationAdapter::remapOptions(optionsMap_t remoteOptions) const {
    ...
    for (auto const &opt : remoteOptions) {
        if (opt.first == "logging_level"sv) {
            ...
            ELOG_DEBUG(logger_, CONFIG, "ElasticDynamicConfigurationAdapter remapOptions Mapped remote 'logging_level->{}' to '{}->{}'.", opt.second, STRINGIFY_HELPER(OTEL_PHP_LOG_LEVEL), loglevel);
        }
        ...
        if (opt.first == "infer_spans") {
            ...
            ELOG_DEBUG(logger_, CONFIG, "ElasticDynamicConfigurationAdapter remapOptions Mapped remote 'infer_spans->{}' to '{}->{}'.", opt.second, STRINGIFY_HELPER(OTEL_PHP_INFERRED_SPANS_ENABLED), result[STRINGIFY_HELPER(OTEL_PHP_INFERRED_SPANS_ENABLED)]);
        }
    }
