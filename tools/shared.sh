...
function adapt_architecture_to_package_type() {
    ...
    case "${package_type}" in
        apk)
            ...
            #    opentelemetry-php-distro_0.3.0_aarch64.apk
            #    opentelemetry-php-distro_0.3.0_x86_64.apk
            adapted_arm64=aarch64
            adapted_x86_64=x86_64
            ;;
        deb)
            # Example for package file names:
            #    opentelemetry-php-distro_0.3.0_amd64.deb
            #    opentelemetry-php-distro_0.3.0_arm64.deb
            adapted_arm64=arm64
            adapted_x86_64=amd64
            ;;
        rpm)
            # Example for package file names:
            #    opentelemetry-php-distro-0.3.0-1.aarch64.rpm
            #    opentelemetry-php-distro-0.3.0-1.x86_64.rpm
            adapted_arm64=aarch64
            adapted_x86_64=x86_64
            ;;
        *)
...
function select_otel_package_file() {
...
function should_pass_env_var_to_docker () {
    ...
    if [[ ${env_var_name_to_check} == "OTEL_PHP_"* ]] || [[ ${env_var_name_to_check} == "OTEL_"* ]]; then
        echo "true"
        return
    fi
