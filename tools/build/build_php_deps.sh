...
verify_otlp_exporters() {
    ...
    if [ "${has_compared_the_same}" = "false" ]; then
        echo "${_VENDOR_DIR}/${_PHP_IMPL_PACKAGE_NAME} content differs from the base"
...
verify_vendor_dir() {
    local _PHP_VERSION_WITHOUT_DOT="${1:?}"
    local _VENDOR_DIR="${2:?}"
    local _OTEL_PROTO_VERSION="${3:?}"
    local _NATIVE_OTLP_EXPORTERS_PROTO_VERSION="${4:?}"
    local _PHP_SCOPER_ENABLED="${5:?}"

    verify_otel_proto_version "${_VENDOR_DIR}" "${_OTEL_PROTO_VERSION}"
    if [ "${_PHP_SCOPER_ENABLED}" = "true" ]; then
        echo "Skipping verify_otlp_exporters because vendor directory is namespace-prefixed by php-scoper"
    else
        verify_otlp_exporters "${_PHP_VERSION_WITHOUT_DOT}" "${_VENDOR_DIR}" "${_NATIVE_OTLP_EXPORTERS_PROTO_VERSION:?}"
    fi
}
...
main() {
    ...
    for _PHP_VERSION_WITHOUT_DOT in "${PHP_VERSIONS_WITHOUT_DOT[@]}"; do
        local _PHP_VERSION_WITH_DOT=$(convert_no_dot_to_dot_separated_version "${_PHP_VERSION_WITHOUT_DOT}")
        ...
        local COMPOSER_LOCK_FILENAME="$(build_generated_composer_lock_file_name "prod" "${_PHP_VERSION_WITHOUT_DOT}")"
        ...
        if [ "${_PROJECT_PROPERTIES_PHP_SCOPER_ENABLED:-false}" = "true" ]; then
            ...
            SCOPER_CMD="\
                ...
                && curl -fLsS -o /usr/local/bin/php-scoper.phar https://github.com/humbug/php-scoper/releases/download/${_PHP_SCOPER_VERSION}/php-scoper.phar \
                && cd /tmp/repo/vendor \
        ...
        docker run --rm \
            ...
            sh -c "\
                ...
                && for entry in *; do [ "\${entry}" = "NOTICE" ] && continue; cp -r "\${entry}" /tmp/repo/; done \
                && cd /tmp/repo/ \
        ...
        if [ "${SKIP_VERIFY}" = "false" ]; then
            verify_vendor_dir "${_PHP_VERSION_WITHOUT_DOT}" "${_VENDOR_DIR}" "${_PROJECT_PROPERTIES_OTEL_PROTO_VERSION}" "${_PROJECT_PROPERTIES_NATIVE_OTLP_EXPORTERS_BASED_ON_PHP_IMPL_VERSION}" "${_PROJECT_PROPERTIES_PHP_SCOPER_ENABLED:-false}"
        fi
    done
}
