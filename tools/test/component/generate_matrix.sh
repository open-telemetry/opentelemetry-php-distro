...
function generate_rows_to_test_sepcial_cases () {
    local php_version_no_dot
    # SC2086: Double quote to prevent globbing and word splitting.
    # shellcheck disable=SC2086
    ...
    echo "${php_version_dot_separated},${package_type},${test_app_code_host_kind},${test_group},OTEL_PHP_LOG_LEVEL_SYSLOG=trace,OTEL_PHP_SCOPED_DEPS_ENABLED=false"
    ...
    echo "${php_version_dot_separated},${package_type},${test_app_code_host_kind},${test_group},OTEL_PHP_LOG_LEVEL_STDERR=debug"

    # SC2086: Double quote to prevent globbing and word splitting.
    # shellcheck disable=SC2086
    php_version_no_dot=$(get_one_before_highest_value_from_array ${_PROJECT_PROPERTIES_SUPPORTED_PHP_VERSIONS})
    php_version_dot_separated=$(convert_no_dot_to_dot_separated_version "${php_version_no_dot}")
    package_type='rpm'
    local test_app_code_host_kind="${test_app_code_host_kinds[0]}"
    echo "${php_version_dot_separated},${package_type},${test_app_code_host_kind},${test_group},OTEL_PHP_SCOPED_DEPS_ENABLED=false"
}
...
function main () {
    ...
    generate_rows_to_test_sepcial_cases
}
