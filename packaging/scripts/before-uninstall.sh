...
function remove_extension_configuration_to_file() {
    HEADER='; THIS IS AN AUTO-GENERATED FILE by the OpenTelemetry PHP Distro post-install.sh script'
    FOOTER='; END OF AUTO-GENERATED by the OpenTelemetry PHP Distro post-install.sh script'
    EXTENSION='extension=.*'
    BOOTSTRAP='opentelemetry_distro.bootstrap_php_part_file=.*'
    ...
    if [ "${numberOfHeaderMatches}" == "1" ] && [ "${numberOfExtensionMatches}" == "1" ] && [ "${numberOfBootstrapMatches}" == "1" ] && [ "${numberOfFooterMatches}" == "1" ] ; then
        ...
        if [ "${extension}" == "2" ] && [ "${bootstrap}" == "3" ] && [ "${footer}" == "4" ] ; then
            echo "$1 has been configured with the OpenTelemetry PHP Distro setup, let's uninstalled it."
            ...
            echo "$1 has been configured with the OpenTelemetry PHP Distro setup. But it cannot be uninstalled automatically."
...
function manual_extension_agent_uninstallation() {
    echo 'Uninstall manually by removing the following lines from your php.ini file:'
    echo '--------------------------------------------------------------'
    echo 'extension=opentelemetry_php_distro_loader.so'
    echo 'opentelemetry_distro.bootstrap_php_part_file=/opt/opentelemetry/php/distro/bootstrap.php'
}
...
function uninstall_conf_d_files() {
    ...
    echo "Uninstalling ${OTEL_INI_FILE_NAME} for supported SAPI's"
    ...
    for SAPI_CONFIG_D_PATH in "${SAPI_CONFIG_DIRS[@]}" ; do
        ...
        unlink_file "${SAPI_CONFIG_D_PATH}/98-${OTEL_INI_FILE_NAME}"
...
############################### MAIN ###########################################
################################################################################
echo 'Uninstalling OpenTelemetry PHP Distro'
...
if [ -e "${PHP_INI_FILE_PATH}" ] ; then
    if grep -q "opentelemetry_php_distro_loader.so" "${PHP_INI_FILE_PATH}" ; then
        remove_extension_configuration_to_file "${PHP_INI_FILE_PATH}"
    else
        echo '  extension configuration does not exist for the OpenTelemetry PHP Distro.'
        echo '  skipping ... '
    fi
else
...
if is_extension_enabled ; then
    echo 'Failed. OpenTelemetry PHP Distro extension is still enabled.'
    if [ -e "${PHP_INI_FILE_PATH}${BACKUP_EXTENSION}" ] ; then
        ...
        echo "${PHP_INI_FILE_PATH} got some leftovers please delete the entries for the OpenTelemetry PHP Distro manually"
    fi
else
    echo 'Extension has been removed successfully for OpenTelemetry PHP Distro.'
fi
